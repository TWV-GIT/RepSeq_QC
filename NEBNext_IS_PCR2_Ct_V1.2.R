...
# 1.2 <- added graph displaying number of cycles required
...
# Set parameters----------------------------------------------------------------
win_dir <- 'C:\\Users\\tverdonckt\\OneDrive - ITG\\Desktop\\DenMark_VLAIO\\Data\\qPCR\\J9\\24-06-27 - 205_208 - PCR2'
...
# samples that should not be plotted (e.g. ladders)
filternames <- c('100', '50', '12.5', '25', '3.13', '6.25', '1.56','NTC')
# Samples to plot uniquely
highlightnames <- c('D','F')
...
save_tables <- function(){
  ...
  write.table(average_sd_n, file = paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"), "_Ct_values_norm_", Ct_factor, "_treshold.tsv"))
}
# save_plots()
save_plots <- function(){
  pdf(file = paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"), "_library_quantification_plots.PDF"), width = 10, height = 7)
